...
.post(async function (req, res) {
    ...
    if (!newDetalle.expedienteEstudiante || !newDetalle.profesor || !newDetalle.materia  || !newDetalle.experienciaGeneral|| !newDetalle.dificultad
        ...
        let sameDetalle = await Review.find({
            profesor: newDetalle.profesor, materia: newDetalle.materia, expedienteEstudiante: newDetalle.expedienteEstudiante
        });
        ...
        if (sameDetalle.length > 0) {
            ...
            let max = 0;
            let detalle = await Review.findOne().sort({  id: -1 });
            ...
            detail = await Detalle.findOne({profesor: newDetalle.profesor, materia: newDetalle.materia});
            ...
            await Detalle.findOneAndUpdate({profesor:newDetalle.profesor, materia: newDetalle.materia}, detail, {
                new: true
              });
